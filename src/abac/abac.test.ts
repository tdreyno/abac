...
describe("abac", () => {
  ...
  it("supports canBatch with variadic requests", async () => {
    const READ = action("read")
    const approveRead = approve(actionIs(READ))

    const authz = enforcer(policy(approveRead))

    const [first, second] = await authz.canBatch(
      {
        action: READ,
        user: {
          id: "u1",
